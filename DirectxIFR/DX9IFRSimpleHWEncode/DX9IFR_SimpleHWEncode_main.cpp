...
#define SAMPLE_NAME "DX9IFR_SimpleHWEncode"
...
unsigned char * g_pBitStreamBuffer;
HANDLE g_EncodeCompletionEvent;

std::string g_sBaseName = "DX9IFR_SimpleHWEncode"; // Default Basename for the output stream
...
INT WINAPI wWinMain( HINSTANCE hInst, HINSTANCE, LPWSTR cmdline, INT )
{
    ...
    if( SUCCEEDED( InitD3D( hWnd ) ) )
    {
        ...
        params.configParams.eRateControl = NV_HW_ENC_PARAMS_RC_2_PASS_FRAMESIZE_CAP;
        ...
        if( SUCCEEDED( InitGeometry() ) )
        {
            ...
            while( msg.message != WM_QUIT && msg.message != WM_DESTROY )
            {
                ...
                {
                    if (dwFrameNumber < g_dwMaxFrames)
                    {
                        dwFrameNumber++;
                        Render(fileOut);
                    }
    ...
    //! Free NvIFR objects, library and scene objects
    if (g_pIFR)
    {
        g_pIFR->NvIFRRelease();
        g_pIFR = NULL;
