 ...
 * framebuffer with dynamically increasing width/height of encoded image,
 * increasing bit rate and both bit rate and resolution.
 *
 * \copyright
...
//! width and height of the encoded stream
static unsigned int encodeWidth = fboWidth, encodeHeight = fboHeight;
//! maximum width and height of the encoded stream
static unsigned int maxWidth = 4096, maxHeight = 4096;
//! frame counter
...
//! output file
FILE *outFile;
static char outFileExt[2][5] = {"h264", "h265"};
//! output file name
static char outFileName[MAX_OUTPUT_FILENAME_LENGTH] = "movie";
...
static void display()
{
    ...
    // Increase width, height and bit rate of encoded image in combinations after each 120 frames.
    if ((frameCounter % 120 == 0) && (frameCounter % 240 != 0) && (frameCounter % 480 != 0))
    {
        ...
        {
            encodeWidth  = fboWidth;
            encodeHeight = fboHeight;
        }

        params.newWidth = encodeWidth;
        params.newHeight = encodeHeight;
    }
    if (frameCounter % 240 == 0)
    {
        params.flags = NV_IFROGL_HW_ENC_PARAM_FLAG_DYN_BITRATE_CHANGE;

        params.newAvgBitRate = calculateBitrate(encodeWidth, encodeHeight);
        params.newVBVBufferSize = params.newAvgBitRate;
        params.newVBVInitialDelay = params.newAvgBitRate;
    }
    ...
    snprintf(textBuffer, sizeof(textBuffer), "%dx%d", encodeWidth, encodeHeight);
    printText(0, winHeight - 12, textBuffer);

    glBindFramebuffer(GL_FRAMEBUFFER, 0);

    // transfer the FBO
    if (nvIFR.nvIFROGLTransferFramebufferToHwEnc(transferObjectHandle, &params, fboID, GL_COLOR_ATTACHMENT0, GL_NONE) != NV_IFROGL_SUCCESS)
    {
...
int main(int argc, char *argv[])
{
    ...
    glutCreateWindow("NvIFROpenGLNVENCDynRes");
    ...
    // The maximum resolution needs to be specified when creating the transfer object
    config.flags = NV_IFROGL_HW_ENC_CONFIG_FLAG_DYN_RESOLUTION_CHANGE;
    config.maxDynamicWidth = maxWidth;
    config.maxDynamicHeight = maxHeight;
    ...
    if (isOutFile)
    {
        sprintf(outFileName, "%s.%s", outFileName, outFileExt[codecType]);
