...
Encoder::Encoder()
    ...
    , m_bIsHEVC(false), m_bIsYUV444(false), m_bDone(false)
{
    // Init members
    ...
    memset(&m_stInputSurface[0], 0, sizeof(EncodeInputSurfaceInfo));
    memset(&m_stBitstreamBuffer[0], 0, sizeof(EncodeOutputBuffer));
...
void Encoder::EncodeThreadProc(void *args)
{
    Encoder *pEncoder = (Encoder *)args;
    Timer encodeTimer;
    for (int frameCnt = 0; frameCnt < pEncoder->m_iFrameCnt; ++frameCnt)
    {
        encodeTimer.reset();
        HRESULT hr = pEncoder->LaunchEncode();
        if (hr != S_OK)
        {
            fprintf(stderr, "Failed encoding via LaunchEncode frame %d\n", frameCnt);
            break;
        }
        hr = pEncoder->GetBitstream(pEncoder->m_fOut);
        if (hr != S_OK)
        {
            fprintf(stderr, "Failed encoding via GetBitstream frame %d\n", frameCnt);
            break;
        }
        double encodeTime = encodeTimer.now();

        printf("Encode %d: frame %d, encode time %.2f\n",
            frameCnt, frameCnt, encodeTime);

        Sleep(15);
    }
    pEncoder->m_bDone = true;
}
    ...
    unsigned int codec, HANDLE hSharedSurface, bool bYUV444, int iFrameCnt, FILE *fOut)
{
    ...
    if (hSharedSurface)
    {
        ...
        m_pD3D9Dev->CreateOffscreenPlainSurface(dwWidth, dwHeight, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &pD3D9Surf, &hSharedSurface);
        if (pD3D9Surf)
        {
            m_stInputSurface[0].pD3DSurf = pD3D9Surf;
            NV_ENC_REGISTER_RESOURCE param = { 0 };
            // Register resource if not already registered
            param.version = NV_ENC_REGISTER_RESOURCE_VER;
            param.resourceToRegister = (void *)m_stInputSurface[0].pD3DSurf;
            ...
            m_stInputSurface[0].hRegisteredHandle = param.registeredResource;
        }
        else
        {
    ...
    _beginthread(EncodeThreadProc, 0, this);
...
HRESULT Encoder::LaunchEncode()
{
    ...
    if (m_stInputSurface[0].hRegisteredHandle == NULL)
    {
        fprintf(stderr, __FUNCTION__": No input resources registered. Quitting.\n");
        return E_FAIL;
    }

    if (m_stInputSurface[0].hInputSurface == NULL)
    {
        NV_ENC_MAP_INPUT_RESOURCE mapParam = { 0 };
        mapParam.version = NV_ENC_MAP_INPUT_RESOURCE_VER;
        mapParam.registeredResource = m_stInputSurface[0].hRegisteredHandle;
        ...
        m_stInputSurface[0].hInputSurface = mapParam.mappedResource;
        m_stInputSurface[0].bufferFmt = mapParam.mappedBufferFmt;
    ...
    m_stEncodePicParams.inputBuffer = m_stInputSurface[0].hInputSurface;
    m_stEncodePicParams.bufferFmt = m_stInputSurface[0].bufferFmt;
    m_stEncodePicParams.inputWidth = m_stInputSurface[0].dwWidth;
    m_stEncodePicParams.inputHeight = m_stInputSurface[0].dwHeight;
    m_stEncodePicParams.outputBitstream = m_stBitstreamBuffer[0].hBitstreamBuffer;
...
HRESULT Encoder::GetBitstream(FILE *fOut)
{
    if (!m_hEncoder)
        return E_FAIL;
    HRESULT hr = S_OK;
    ...
    if (m_bUseMappedResource)
    {
        ...
        nvStatus = m_pEncodeAPI->nvEncUnmapInputResource(m_hEncoder, m_stInputSurface[0].hInputSurface);
        m_stInputSurface[0].hInputSurface = NULL;
    }
    if (m_dwFrameCount == 0)
    {
        m_dwFrameCount++;
    ...
    lockBitstreamData.outputBitstream = m_stBitstreamBuffer[0].hBitstreamBuffer;
    ...
    nvStatus = m_pEncodeAPI->nvEncUnlockBitstream(m_hEncoder, m_stBitstreamBuffer[0].hBitstreamBuffer);
    if (nvStatus != NV_ENC_SUCCESS)
    {
...
HRESULT Encoder::AllocateOPBufs()
{
    ...
    m_stInputSurface[0].dwWidth = m_dwWidth;
    m_stInputSurface[0].dwHeight = m_dwHeight;

    m_stBitstreamBuffer[0].dwSize = 1024 * 1024;
    ...
    m_stBitstreamBuffer[0].hBitstreamBuffer = stAllocBitstream.bitstreamBuffer;
    m_stBitstreamBuffer[0].pBitstreamBufferPtr = stAllocBitstream.bitstreamBufferPtr;
...
HRESULT Encoder::ReleaseIPBufs()
{
    ...
    if (m_stInputSurface[0].hRegisteredHandle)
        status = m_pEncodeAPI->nvEncUnregisterResource(m_hEncoder, m_stInputSurface[0].hRegisteredHandle);
...
HRESULT Encoder::ReleaseOPBufs()
{
    ...
    // Destroy allocated buffers, unregister registered buffers
    if (m_stBitstreamBuffer[0].hBitstreamBuffer)
        status = m_pEncodeAPI->nvEncDestroyBitstreamBuffer(m_hEncoder, m_stBitstreamBuffer[0].hBitstreamBuffer);
    memset(&m_stBitstreamBuffer[0], 0, sizeof(EncodeOutputBuffer));
